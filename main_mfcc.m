...
filename = 'training signal.wav';
%audiowrite(filename,audioTraining(range),Fs);
...
filename2 = 'training+noise signal.wav';
%audiowrite(filename2,audioTrainingNoisy(range),Fs);
...
afe4 = audioFeatureExtractor('SampleRate',Fs, ...
    ...
    'harmonicRatio',true, ...
    'mfcc',true);
featuresTraining4 = extract(afe4,audioTrainingNoisy);

[numWindows4,numFeatures4] = size(featuresTraining4);

M = mean(featuresTraining4,1);
S = std(featuresTraining4,[],1);
featuresTraining4 = (featuresTraining4 - M) ./ S;

featuresValidation = extract(afe4,audioValidationNoisy);
featuresValidation = (featuresValidation - mean(featuresValidation,1)) ./ std(featuresValidation,[],1);

windowLength = numel(afe4.Window);
hopLength = windowLength - afe4.OverlapLength;
range4 = (hopLength) * (1:size(featuresTraining4,1)) + hopLength;
maskMode = zeros(size(range4));
for index = 1:numel(range4)
...
range5 = (hopLength) * (1:size(featuresValidation,1)) + hopLength;
maskMode = zeros(size(range5));
for index = 1:numel(range5)
...
trainFeatureCell = helperFeatureVector2Sequence(featuresTraining4',sequenceLength,sequenceOverlap);
...
featuresTest = extract(afe4,audioTestNoisy);
